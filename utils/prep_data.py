...
def train_val_split(ictal_X, ictal_y, interictal_X, interictal_y, val_ratio):

    if isinstance(interictal_y, list):
        interictal_X = np.concatenate(interictal_X,axis=0)
        interictal_y = np.concatenate(interictal_y,axis=0)
    if isinstance(ictal_y, list):
    ...
    are balanced
    '''
    down_spl = int(np.floor(interictal_y.shape[0]/ictal_y.shape[0]))
    if down_spl > 1:
        interictal_X = interictal_X[::down_spl]
        interictal_y = interictal_y[::down_spl]
    elif down_spl == 1:
        interictal_X = interictal_X[:ictal_X.shape[0]]
        interictal_y = interictal_y[:ictal_X.shape[0]]

    print ('Balancing:', ictal_y.shape,interictal_y.shape)

    #ictal_X = shuffle(ictal_X,random_state=0)
    #interictal_X = shuffle(interictal_X,random_state=0)
    X_train = np.concatenate((ictal_X[:int(ictal_X.shape[0]*(1-val_ratio))],interictal_X[:int(interictal_X.shape[0]*(1-val_ratio))]),axis=0)
    y_train = np.concatenate((ictal_y[:int(ictal_X.shape[0]*(1-val_ratio))], interictal_y[:int(interictal_X.shape[0]*(1-val_ratio))]),axis=0)

    X_val = np.concatenate((ictal_X[int(ictal_X.shape[0]*(1-val_ratio)):],interictal_X[int(interictal_X.shape[0]*(1-val_ratio)):]),axis=0)
    y_val = np.concatenate((ictal_y[int(ictal_X.shape[0]*(1-val_ratio)):], interictal_y[int(interictal_X.shape[0]*(1-val_ratio)):]),axis=0)
